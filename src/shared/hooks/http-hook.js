...
export const useHttpClient = () => {
  ...
  //UseCallback prevents the infinite loops (otherwise this function will be re-created each times, when component re-rendered)
  ...
  useEffect(() => {
    return () => {
      activeHttpRequests.current.forEach((abortCtrl) => abortCtrl.abort()); //abort method stops the request
    };
  }, []);
